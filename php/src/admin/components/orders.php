...
<div class="d-flex justify-content-center">
    <ul class="m-0 p-0 list-unstyled d-flex flex-row gap-4">
        ...
        foreach ($ordersTabsName as $index => $tabname) { ?>
            <a href="/admin/?page=orders&tabs=<?= $tabname; ?>" class="text-decoration-none text-orange fs-5 fw-medium btn rounded-3 <?= isset($_GET["tabs"]) && $_GET["tabs"] == $tabname ? "active btn-outline-orange" : "btn-light" ?>">
                <li>
                    <?php
...
<div class="mt-4 p-2 overflow-y-auto overflow-x-hidden" style="height: calc(100% - 15%);">
    ...
    if ($items->rowCount() > 0) {
        while ($item = $items->fetch()) {
            ...
            <div class="bg-white w-100 p-4 rounded-3 shadow-sm border mb-5 position-relative">
                <?php //if ($item["status"] == "canceled") { 
                ?>
                <!-- <div class="d-flex w-100 justify-content-center">
                        <button type="button" class="d-flex align-items-center gap-2 btn mb-3">
                            <div class="btn btn-danger rounded-circle">
                                <i class="fa-solid fa-trash-can"></i>
                            </div>
                            <div class="text-danger fs-5">ลบประวัตินี้</div>
                        </button>
                    </div> -->
                <?php //} 
                ?>
                ...
                <div class="d-flex align-items-center" style="gap:10px">
                    ...
                    if ($item["status"] == "waiting") { ?>
                        <a href="../api/manage_orders.php?cancelOrder&order_id=<?= $item["order_id"] ?>" class="btn btn-danger d-flex align-items-center w-100 justify-content-center" style="gap:10px"><i class="fa-solid fa-circle-xmark"></i> ยกเลิก</a>
                        <a href="../api/manage_orders.php?deliveryOrder&order_id=<?= $item["order_id"] ?>" class="btn btn-warning d-flex align-items-center w-100 justify-content-center" style="gap:10px">
                    ...
                    <?php } elseif ($item["status"] == "delivering") { ?>
                        <a href="../api/manage_orders.php?successfullyOrder&order_id=<?= $item["order_id"] ?>" class="btn btn-success d-flex align-items-center w-100 justify-content-center" style="gap:10px"><i class="fa-solid fa-circle-check"></i> จัดส่งสำเร็จ</a>
                    <?php } ?>
                </div>
            </div>
